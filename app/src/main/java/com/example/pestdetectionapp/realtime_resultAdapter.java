...
public class realtime_resultAdapter extends RecyclerView.Adapter<realtime_result_viewHolder> {
    ...
    @Override
    public void onBindViewHolder(@NonNull final realtime_result_viewHolder sviewHolder, @SuppressLint("RecyclerView") final int position3) {
        final int index = sviewHolder.getAdapterPosition();
        sviewHolder.Image.setImageBitmap(list.get(position3).Image);
        sviewHolder.name.setText(list.get(position3).name);
        sviewHolder.scientific.setText(list.get(position3).scientific);
        sviewHolder.view.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view)
            {
                realtime_resultData selectedItem = list.get(position3);
